...
string RecordMod(string div[], string sp, int l, int llinea)
{
    str r;
    r.comune = div[0];
    r.prov = div[1];
    ...
    //stabiliamo una lunghezza fissa per ogni record
    ostringstream rec;
    rec << r.comune << sp << r.prov << sp << r.reg << sp << r.tip << sp << r.stelle << sp << r.den << sp << r.ind << sp << r.cap << sp << r.local << sp << r.fraz << sp << r.tel << sp << r.fax << sp << r.posel << sp << r.web << sp << r.ces << sp << r.cam << sp << r.pls << sp << r.pla << sp << r.mval << sp << r.c << ']' << setw(l - llinea) << "##\r\n";
    return rec.str();
}
...
void ModificaCampo(int campo, string ricerca, string modifica)
{
    ...
    while (getline(file, m))
    {
        ...
        app << RecordMod(div, ";", 300, m.length());
    }
}

int main()
{
    ModificaCampo(0, "CIAO", "APRICALE");
    return 0;
}